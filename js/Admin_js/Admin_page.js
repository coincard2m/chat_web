...
import { loadReportsAndTeacherData } from '/Data_firebase/Reports_data.js';
...
const currentTeacherClass = localStorage.getItem('currentTeacherClass') || '';

// KẾT NỐI FIREBASE REALTIME
document.addEventListener('DOMContentLoaded', () => {
    loadReportsAndTeacherData((reportsList) => {
        // Lưu dữ liệu nhận được từ file data vào biến toàn cục
        allReports = reportsList;
        
        // Tiến hành cập nhật giao diện
        updateStatsUI();
        renderReports();
        updateChartFromData();
    });
...
// === CẬP NHẬT BIỂU ĐỒ TỪ DỮ LIỆU FIREBASE ===
function updateChartFromData() {
    ...
    allReports.forEach(rep => {
        ...
        const idx = Math.min(Math.max(month - (new Date().getMonth() - 5), 0), 5);
...
function renderReports() {
    ...
    const TeacherClass  = localStorage.getItem('UserClass').trim().toLowerCase();

    const filtered = allReports.filter(rep => {
        const studentClass = (rep.studentClass || '').trim().toLowerCase();
        if (TeacherClass !== studentClass) return false;
    ...
    if (filtered.length === 0) {
        container.innerHTML = `
            <div class="text-center py-12 bg-white dark:bg-[#211F26] rounded-3xl border border-gray-100 dark:border-[#49454F]/50 shadow-3xs">
    ...
    filtered.forEach(rep => {
        ...
        const classDisplay = rep.studentClass || rep.senderClass || '—';

        const cardHtml = `
            <div onclick="openDetailModal('${rep.id || rep._key}')" class="bg-white dark:bg-[#211F26] border border-gray-100 dark:border-[#49454F]/50 hover:border-indigo-200 rounded-3xl p-5 md:p-6 shadow-3xs hover:shadow-2xs transition duration-200 cursor-pointer text-left flex flex-col justify-between gap-3 relative overflow-hidden">
                <div class="space-y-2">
                    <div class="flex items-center justify-between gap-2">
                        <div class="flex items-center gap-2 flex-wrap">
                            <span class="text-xs text-gray-400 font-mono font-bold">${rep.id || rep._key}</span>
...
window.filterReports = function(status) {
    ...
    if (activeBtn) {
        activeBtn.className = "filter-btn px-3.5 py-1.5 bg-indigo-600 text-white text-xs font-bold rounded-xl transition";
    }
    renderReports();
};
...
function updateStatsUI() {
    ...
    const currentTeacherClass = (localStorage.getItem('UserClass') || '').trim().toLowerCase();

    // Chỉ lọc theo LỚP của giáo viên, độc lập với các nút tab đang chọn để các con số tổng luôn chính xác
    const classReports = allReports.filter(rep => {
        const reportClass = (rep.studentClass || rep.senderClass || '').trim().toLowerCase();
        if (reportClass !== currentTeacherClass) return false;
        
        return true;
    });

    const total = classReports.length;
    const urgent = classReports.filter(r => r.priority === 'urgent').length;
    const pending = classReports.filter(r => r.status === 'processing').length;
    const resolved = classReports.filter(r => r.status === 'resolved').length;
...
window.openDetailModal = function(id) {
    ...
    document.getElementById('modal-sender-name').textContent = rep.isAnonymous !== false ? "🔒 Ẩn danh học sinh bảo mật" : (rep.senderName || rep.senderId);
    document.getElementById('modal-sender-class').textContent = rep.studentClass || rep.senderClass || '—';
...
// === LƯU CẬP NHẬT TRẠNG THÁI → FIREBASE ===
window.submitStatusUpdate = function() {
    ...
    const reportRef = ref(db, `reports/${currentSelectedReportId}`);
    update(reportRef, {
        status: newStatus,
        ...
        closeDetailModal();
        alert(`✅ Đã cập nhật thành công báo cáo!`);
    }).catch(err => {
...
window.postAnnouncement = function(e) {
    ...
    const receiver = document.getElementById('ann-receiver') ? document.getElementById('ann-receiver').value : 'Tất cả mọi người';
    const category = document.getElementById('ann-category').value;
    const summary = document.getElementById('ann-summary').value.trim();
    const body = document.getElementById('ann-body').value.trim();

    if (!title || !summary || !body) {
        alert('Vui lòng điền đầy đủ thông tin để công bố thông báo!');
        return;
    }
    ...
    const newNotif = {
        title,
        receiver: receiver || 'Tất cả mọi người',
        category,
        summary,
        body,
    ...
    push(notifRef, newNotif).then(() => {
        const successDiv = document.getElementById('announce-success');
        if (successDiv) {
