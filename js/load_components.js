async function loadSetting_Page(containerId = "modal-container") {
    try {
        ...
        const response = await fetch("/components/Setting_component.html");
...
async function loadProfileView(containerId = "modal-container") {
    try {
        // Sử dụng dấu "/" ở đầu để đảm bảo dù đứng ở trang nào cũng load đúng từ thư mục gốc
        const response = await fetch("/components/Account_component.html");
...
// Tải form báo cáo
async function loadReportForm(containerId = "modal-container") {
    try {
        // Sử dụng dấu "/" ở đầu để đảm bảo dù đứng ở trang nào cũng load đúng từ thư mục gốc
        const response = await fetch("/components/Report.html");
        
        if (!response.ok) throw new Error("Không tải được form báo cáo");
        ...
        console.error("Lỗi load form báo cáo:", error);
        document.getElementById(containerId).innerHTML = `
            <p class="text-red-500 p-4">Không thể tải form báo cáo. Vui lòng thử lại sau.</p>
        `;
    }
}

async function loadTracker(containerId = "modal-container") {
    try {
        // Sử dụng dấu "/" ở đầu để đảm bảo dù đứng ở trang nào cũng load đúng từ thư mục gốc
        const response = await fetch("/components/Tracker.html");
        ...
        // Khởi tạo JS cho form sau khi load
        if (typeof initReportTracker === "function") {
            initReportTracker();
        }
        ...
        console.error("Lỗi load form báo cáo:", error);
        document.getElementById(containerId).innerHTML = `
            <p class="text-red-500 p-4">Không thể tải màn hình tra cứu. Vui lòng thử lại sau.</p>
        `;
    }
}
...
window.loadSetting_Page = loadSetting_Page;