async function loadReportForm(containerId = "modal-container") {
    try {
        ...
        const response = await fetch("/Parent_components/Report_parents.html");
        
        if (!response.ok) throw new Error("Không tải được form phản ánh");
        ...
        console.error("Lỗi load form phản ánh:", error);
        document.getElementById(containerId).innerHTML = `
            <p class="text-red-500 p-4">Không thể tải form phản ánh. Vui lòng thử lại sau.</p>
        `;
    }
}
...
async function loadProfileView(containerId = "modal-container") {
    try {
        // Sử dụng dấu "/" ở đầu để đảm bảo dù đứng ở trang nào cũng load đúng từ thư mục gốc
        const response = await fetch("/Parent_components/Account_parents.html");
...
document.addEventListener("DOMContentLoaded", () => {
    loadComponent("navbar", "Parent_components/navbar.html");
    loadComponent("footer", "Parent_components/footer.html");
});
